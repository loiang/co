...
- When you cannot make that API change and still need a small positional-literal callsite in Rust, follow the `argument_comment_lint` convention:
  ...
  - You can run `bazel build --config=argument-comment-lint -- $(tools/argument-comment-lint/list-bazel-targets.sh)` from the repository root to run the lint check locally. This is powered by Bazel, so running it the first time can be slow if Bazel is not warmed up, though incremental invocations should take <15s. Most of the time, it is best to update the PR and let CI take responsibility for checking this (or run it asynchronously in the background after submitting the PR). Note CI checks all three platforms, which the local run does not.
...
- If you change `ConfigToml` or nested config types, run `cargo run -p codex-config-schema --bin codex-write-config-schema` from `codex-rs` to update `codex-rs/core/config.schema.json`.
...
- If you change Rust dependencies (`Cargo.toml` or `Cargo.lock`), run `bazel mod deps --lockfile_mode=update` from the
...
- When running Rust commands (for example `cargo clippy` or `cargo nextest`) be patient with the command and never try to kill them using the PID. Rust lock can make the execution slow, this is expected.

Run `python3 scripts/format.py` from the repository root automatically after you have finished making code changes anywhere in this repository; do not ask for approval to run it. Additionally, run the tests:

1. Do not run `cargo test` directly. Use `RUST_MIN_STACK=8388608 NEXTEST_PROFILE=local cargo nextest run --no-fail-fast` from `codex-rs` so test execution follows the repo defaults.
2. Run the test for the specific project that was changed. For example, if changes were made in `codex-rs/tui`, append `-p codex-tui` to the nextest command.
3. Once those pass, if any changes were made in common, core, or protocol, run the complete nextest command. Avoid `--all-features` for routine local runs because it expands the build matrix and can significantly increase `target/` disk usage; use it only when you specifically need full feature coverage. Project-specific or individual tests can be run without asking the user, but do ask the user before running the complete test suite.

Before finalizing a large change to `codex-rs`, run `cargo clippy --fix --tests --allow-dirty -p <project>` from `codex-rs` to fix any linter issues in the code. Prefer scoping with `-p` to avoid slow workspace-wide Clippy builds; omit `-p` only if you changed shared crates. Do not re-run tests after running Clippy fix or formatting.
...
cargo benchmarks can be run with `cargo bench --workspace --bench '*'`, use the divan crate to write new ones.

Use `cargo bench --workspace --bench '*' -- --test` to dry-run the benchmark for a single iteration to ensure it works.
...
- Regenerate schema fixtures when API shapes change:
  ...
  (and append `--experimental` when experimental API fixtures are affected).
- Validate with `RUST_MIN_STACK=8388608 NEXTEST_PROFILE=local cargo nextest run --no-fail-fast -p codex-app-server-protocol`.
